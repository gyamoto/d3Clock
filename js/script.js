...
// 	d3.select("svg").append("g").attr("class", "years")
// 		.append("text").attr("class", "label")
// 		.attr("x", 20).attr("y", 20);
// 	d3.select("svg").append("g").attr("class", "months");
// 	d3.select("svg").append("g").attr("class", "days");
// 	d3.select("svg").append("g").attr("class", "hours");
// 	d3.select("svg").append("g").attr("class", "minutes");
// 	d3.select("svg").append("g").attr("class", "seconds");
// }
...
// 	var nowYear = d3.select("g.years text");
// 	var oldYear = nowYear.text();
// 	if(oldYear != years) {
// 		nowYear.transition().duration(delay).style("opacity", 0)
// 			.transition().duration(delay).style("opacity", 1).text(years);
// 	}
// }
// function drawMonth(months) {
// 	var rects = new Array(months);
// 	for(var i=0; i<rects.length; i++) { rects[i] = i; }
// 	d3.select("svg g.months")
// 		.selectAll("rect").data(rects, function(d){ return d; })
// 			.exit().remove();
// 	d3.select("svg g.months")
// 		.selectAll("rect").data(rects, function(d){ return d; })
// 			.enter().append("rect")
// 			.attr("fill", "#ffaaaa")
// 			.attr("cx", function(d, i, n) { return (i+1) * 20; })
// 			.attr("cy", 20)
// 			.attr("width", 10)
// 			.attr("height", 40);
// }